...
`v0.3.0`. Read it completely before changing the machine. If the plugin is not installed, stop and
...
- Clone and run the exact `v0.3.0` release updater. Do not execute an updater directly from `main`.
...
git clone --depth 1 --branch v0.3.0 https://github.com/yuyou-dev/GrillMeJewel.git "$UPDATE_ROOT"
...
Continue only when the exact tag is `v0.3.0` and `package.json` reports `0.3.0`.
...
The updater replaces the old fixed release ref with `v0.3.0`, restores the single plugin, and
...
Confirm `grill-me-jewel@grill-me-jewel` is installed and enabled at `0.3.0`, and doctor is `ready`
